...
class ekQuizzes_CPT
{
	...
	var $defaultQuizOptions = array(
		...
		"emailAdminOnCompletion"	=> "", // Email current user when quiz is finised
...
/*	---------------------------
	...
	function  drawOptionsMetaBox($post,$metabox)
	{
		...
		$emailAdminOnCompletion = isset( $quizMeta['emailAdminOnCompletion'] ) ? $quizMeta['emailAdminOnCompletion'][0] : $defaultQuizOptions['emailAdminOnCompletion'];	
		...
		echo '<hr/>';
		echo '<label for="emailAdminOnCompletion"><input type="checkbox" id="emailAdminOnCompletion" name="emailAdminOnCompletion" ';
		if($emailAdminOnCompletion=="on"){echo ' checked ';}
		echo '/>Email me when a participant has taken the quiz</label>';
